...
public class BuyOrderService {
    ...
    public BuyOrderService(BuyOrderRepository buyOrderRepository, EmployeeService employeeService, LendingOrderToolRepository lendingOrderToolRepository,
    ...
    public void deleteBuyOrder(Long id) throws IllegalArgumentException {
        buyOrderRepository.findById(id)
                .ifPresent(buyOrder -> {
    ...
    protected void decreaseToolCountIfWasInPreviousBuyOrder(Set<BuyOrderTool> previousBuyOrderTools, Tool tool) {
        previousBuyOrderTools.stream()
                .filter(buyOrderTool ->
                        buyOrderTool.getTool().getId().equals(tool.getId()))
                .findFirst()
                .ifPresent(buyOrderTool ->
                        backToToolCountBeforePurchase(buyOrderTool.getCount(), tool));
    ...
    protected boolean isWasAnyLendingOperationAfterBuyOrderOnTool(Tool tool, Timestamp additionToBuyOrder) {
        return lendingOrderToolRepository.findAllByTool(tool)
                .stream()
                .anyMatch(lendingOrderTool ->
                        lendingOrderTool.getLendingOrder().getPickTime().after(additionToBuyOrder));
    }
    ...
    protected boolean isWasAnyDestructionOperationAfterBuyOrderOnTool(Tool tool, Timestamp additionToBuyOrder) {
        return destructionOrderToolRepository.findAllByTool(tool)
                .stream()
                .anyMatch(destructionOrderTool ->
                        destructionOrderTool.getDestructionOrder().getAddTimestamp().after(additionToBuyOrder));
    }
